...
export default defineComponent({
  ...
  components: {
    BuildingLabel, 
    CusModal,
    // CusModal: defineAsyncComponent (()=> import('../CusModal.vue'))
     EquipmentComp
  },
  props: {
    videoSrc: {
  ...
  setup(props) {
    ...
    const handlePopupClick = async (popup: any) => {
      // 处理弹窗点击事件
      curClickPopupContent.value = popup;
      await nextTick();
      cusmodalRef.value.methods.openModal(popup);
    };
      ...
      buildings.forEach(building => {
        ...
        const app = createApp(BuildingLabel, {
          ...
          onLabelClick: (name: string) => {
            ...
            // router.push({ path: 'third', query: { typename: building.name }})
            // 在这里添加你想要的标签点击处理逻辑
          }
        });
        appList.push(app)
